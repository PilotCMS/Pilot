(() => {
  if (window.__tweakerInAppLoaded) {
    return;
  }

  window.__tweakerInAppLoaded = true;
...
`;
  ...
  const getDataBinding = (element) => {
    let current = element;
    while (current && current !== document.body) {
      if (current.dataset?.tweakerModel && current.dataset?.tweakerId && current.dataset?.tweakerField) {
        return {
          model: current.dataset.tweakerModel,
          id: current.dataset.tweakerId,
          field: current.dataset.tweakerField,
        };
      }
  ...
  const saveChanges = async (kind) => {
    if (lockedSelectors.length === 0) {
      return;
    }

    const rules = lockedSelectors.map((selector) => {
      const declarations = {};
      overrides.forEach((value, property) => {
        if (value.trim()) {
    ...
    const edits = Array.from(textEdits.entries()).map(([selector, text]) => ({
      selector,
      text,
    }));

    const csrf = getCsrfToken();
    const response = await fetch('/_tweaker/save', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      console.warn('Tweaker save failed', await response.text());
    }
  };

  const saveModelBinding = async (binding, value) => {
    if (!binding) {
      return;
    }

    const csrf = getCsrfToken();
    const response = await fetch('/_tweaker/model', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(csrf ? { 'X-CSRF-TOKEN': csrf } : {}),
    ...
    if (!response.ok) {
      console.warn('Tweaker model update failed', await response.text());
    }
  };
  ...
  const updateSelectedLabel = () => {
    ...
    const bindingSummary = dataBinding
      ? `DB: ${dataBinding.model}#${dataBinding.id}.${dataBinding.field}`
      : 'DB: none';
    state.selectedMeta.textContent = `Locked: ${selectorSummary} | ${bindingSummary}`;
  };
  ...
  const lockSelection = (element, options = { append: false }) => {
    ...
    dataBinding = getDataBinding(element);
    overrides = new Map();
    refreshLockedElements();
    renderControls();
  ...
  const createPanel = () => {
    ...
    const saveLessButton = document.createElement('button');
    saveLessButton.type = 'button';
    saveLessButton.className = 'dialkit-button dialkit-button-ghost';
    saveLessButton.textContent = 'Save LESS';
    saveLessButton.addEventListener('click', () => saveChanges('less'));

    const saveBladeButton = document.createElement('button');
    saveBladeButton.type = 'button';
    saveBladeButton.className = 'dialkit-button dialkit-button-ghost';
    saveBladeButton.textContent = 'Save Blade';
    saveBladeButton.addEventListener('click', () => saveChanges('blade'));
    ...
    controls.appendChild(saveLessButton);
    controls.appendChild(saveBladeButton);
  ...
  const onDoubleClick = (event) => {
    ...
    const onBlur = () => {
      ...
      if (selector) {
        textEdits.set(selector, target.textContent ?? '');
      }
      const binding = getDataBinding(target);
      if (binding) {
        saveModelBinding(binding, target.textContent ?? '');
      }
  ...
  window.Tweaker = { toggle, activate, deactivate };
})();
